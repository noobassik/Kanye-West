class ResizeImageForOtherActiveNotModeratedProperty < ActiveRecord::Migration[5.2]
  def change

    big = [2510769, 146669, 732800, 3175395]
    properties = Property.where("properties.is_active = ? AND properties.moderated = ?", true, false).where.not(country_id: big)
